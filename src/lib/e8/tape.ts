import type { AssetClass } from "./instruments.ts";
import { HIP3_INSTRUMENTS } from "./instruments.ts";
...
export const CLASS_RANGE: Record<AssetClass, { range4h: number; range24h: number }> = {
...
export function tapeFor(symbol: string, snap: TapeSnapshot): MarketTape | undefined {
  ...
  const direct = snap.markets[k] ?? snap.markets[symbol];
  if (direct) return direct;
  for (const m of Object.values(snap.markets)) {
    if (m.symbol.toUpperCase() === k || m.hl.toUpperCase() === k) return m;
  }
  return undefined;
}
